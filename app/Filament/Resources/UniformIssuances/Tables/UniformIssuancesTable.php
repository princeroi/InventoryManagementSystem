...
class UniformIssuancesTable
{
    ...
    // -------------------------------------------------------------------------
    // Variant Map — single query instead of N queries
    // -------------------------------------------------------------------------
        ...
        return $table
            ...
            ->recordActions([
                ...
                ActionGroup::make([
                    ...
                    // ── RELEASE (pending/partial → released/partial) ──────────
                    Action::make('release')
                        ->label('Release')
                        ->color('info')
                        ...
                        ->visible(fn ($record) =>
                            ...
                            self::userCan('release uniform-issuance')
                        )
                        ...
                        ->form(function ($record): array {
                            ...
                            foreach ($record->recipients as $recipient) {
                                ...
                                foreach ($recipient->items as $issuanceItem) {
                                    ...
                                    $remaining = $issuanceItem->remaining_quantity > 0
                                        ? $issuanceItem->remaining_quantity
                        ...
                        ->action(function ($record, array $data) {
                            ...
                            foreach ($record->recipients as $recipient) {
                                foreach ($recipient->items as $issuanceItem) {
                                    $releaseQty = (int) ($data["qty_{$issuanceItem->id}"] ?? 0);
                                    ...
                                    if ($releaseQty > $stock) {
                                        $itemName       = $issuanceItem->item?->name ?? "Item #{$issuanceItem->item_id}";
                                        $insufficient[] = "{$itemName} ({$issuanceItem->size}): needs {$releaseQty}, has {$stock}";
                                    }
                                }
                            }
                            ...
                            if (! empty($insufficient)) {
                                Notification::make()
                                    ...
                                    ->body('Cannot release: ' . implode(' | ', $insufficient))
                                    ->danger()
                                    ->persistent()
                                    ->send();
                                return;
                            ...
                            // Deduct stock and update item tracking
                            foreach ($record->recipients as $recipient) {
                                foreach ($recipient->items as $issuanceItem) {
                                    $remaining    = $issuanceItem->remaining_quantity > 0
                                        ? $issuanceItem->remaining_quantity
                                    ...
                                    if ($newRemaining > 0) $allReleased = false;
                            ...
                            $msg = $allReleased
                                ? 'Issuance fully released. Stock deducted.'
                                : 'Issuance partially released. Stock deducted.';
                    ...
                    Action::make('return')
                        ...
                        ->action(function ($record, array $data) {
                            ...
                            foreach ($record->recipients as $recipient) {
                                foreach ($recipient->items as $issuanceItem) {
                                    $returnQty = (int) ($data["qty_{$issuanceItem->id}"]
                                        ?? $issuanceItem->released_quantity
                                        ?? $issuanceItem->quantity);

                                    if ($returnQty <= 0) continue;

                                    // Restore stock if toggled
                                    if ($restoreStock) {
                                        $key     = "{$issuanceItem->item_id}:{$issuanceItem->size}";
                                        $variant = $variantMap[$key] ?? null;
                                    ...
                                    // Record each returned item in the return_items table
                            ...
                            $msg = $restoreStock
                                ? 'Issuance returned. Stock restored.'
                    ...
                    ->form(function ($record): array {
                        ...
                        foreach ($recipients as $i => $recipient) {
                            ...
                            $statusBadge = match ($record->status) {
                                ...
                                'released'  => ['label' => 'Released',  'bg' => '#f0f9ff', 'border' => '#bae6fd', 'color' => '#0284c7'],
                            ...
                            $thReturned = $isReturned
                                ? "<th style='padding:9px 14px;text-align:center;font-size:11px;font-weight:700;color:#dc2626;text-transform:uppercase;'>Returned</th>"
                                : '';

                            // ── Build return map keyed by item_id:size ────────
                            // Sums all return_item rows for this recipient
                            $returnMap = [];
                            ...
                            $summaryReturnedHtml = $isReturned
                                ? "
                                    <div style='width:1px;background:#e5e7eb;align-self:stretch;'></div>
                                    <div style='text-align:center;'>
                                        <div style='font-size:16px;font-weight:800;color:#dc2626;'>{$totalReturned}</div>
                                        <div style='font-size:10px;color:#9ca3af;text-transform:uppercase;letter-spacing:0.05em;margin-top:1px;'>Returned</div>
                                    </div>
                                  "
                                : '';
                            ...
                            $summaryHtml = "
                                ...
                                '>
                                    ...
                                    <div style='text-align:center;'>
                                        <div style='font-size:16px;font-weight:800;color:#1d4ed8;'>{$totalOrdered}</div>
                                        <div style='font-size:10px;color:#9ca3af;text-transform:uppercase;letter-spacing:0.05em;margin-top:1px;'>Ordered</div>
                                    </div>
                                    <div style='width:1px;background:#e5e7eb;align-self:stretch;'></div>
                                    <div style='text-align:center;'>
                                        <div style='font-size:16px;font-weight:800;color:#059669;'>{$totalReleased}</div>
                                        <div style='font-size:10px;color:#9ca3af;text-transform:uppercase;letter-spacing:0.05em;margin-top:1px;'>Released</div>
                                    </div>
                                    {$summaryReturnedHtml}
                                </div>
                            ";
                                        ...
                                        <table style='width:100%;border-collapse:collapse;'>
                                            <thead>
                                                <tr style='background:#f1f5f9;'>
                                                    ...
                                                    <th style='padding:9px 14px;text-align:center;font-size:11px;font-weight:700;color:#059669;text-transform:uppercase;'>Released</th>
                ...
                Action::make('view_logs')
                    ...
                    ->form(function ($record): array {
                        ...
                        if ($logs->isEmpty()) {
                            return [
                                Placeholder::make('no_logs')
                                    ->label('')
                                    ->content(new HtmlString("<div style='text-align:center;padding:32px 0;color:#9ca3af;font-size:13px;'>No activity logs found.</div>")),
                            ];
                        }
                        ...
                        $config = [
                            ...
                            'released'  => ['color' => '#0284c7', 'bg' => '#f0f9ff', 'border' => '#bae6fd', 'label' => 'Released',  'icon' => 'M5 13l4 4L19 7'],
                            ...
                            if ($rawNote) {
                                ...
                                if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                                    ...
                                    foreach ($decoded as $item) {
                                        ...
                                        if ($ordered !== null) {
                                            $chips .= "<span style='background:#f3f4f6;border:1px solid #e5e7eb;border-radius:999px;padding:1px 8px;font-size:11px;color:#6b7280;font-weight:500;'>📦 {$ordered} ordered</span> ";
                                        }

                                        if (isset($item['released'])) {
                                            $chips .= "<span style='background:#eff6ff;border:1px solid #bfdbfe;border-radius:999px;padding:1px 8px;font-size:11px;color:#2563eb;font-weight:600;'>📤 {$item['released']} released</span> ";
            ...
            ->toolbarActions([
                BulkActionGroup::make([
                    ...
                    BulkAction::make('bulk_release')
                        ...
                        ->color('info')
                        ->icon('heroicon-s-truck')
                        ->visible(fn () => self::userCan('release uniform-issuance'))
                        ->requiresConfirmation()
                        ->modalHeading('Bulk Release')
                        ->modalDescription(fn ($records) =>
                            'Only pending issuances will be released. ' .
                        ...
                        ->action(function (Collection $records) {
                            ...
                            foreach ($eligible as $record) {
                                ...
                                if (! empty($insufficient)) {
                                    $skipped[] = "Issuance #{$record->id}: " . implode(', ', $insufficient);
                                    continue;
                                }
                            ...
                            if ($released > 0) {
                                Notification::make()
                                    ->title("{$released} issuance(s) released. Stock deducted.")
                                    ->success()
                                    ->send();
                            }
                            ...
                            if (! empty($skipped)) {
                                Notification::make()
                                    ->title('Some skipped due to insufficient stock')
                    ...
                    BulkAction::make('bulk_issue')
                        ...
                        ->modalDescription(fn ($records) =>
                            'Only released issuances will be marked as issued. ' .
                            $records->where('status', '!=', 'released')->count() . ' record(s) will be skipped.'
                        )
                        ->action(function (Collection $records) {
                            $eligible = $records->where('status', 'released');
                            foreach ($eligible as $record) {
                                $record->update(['status' => 'issued', 'issued_at' => now()]);
                            }
                            $skipped = $records->count() - $eligible->count();
                            Notification::make()
                                ->title("{$eligible->count()} issuance(s) marked as issued." . ($skipped > 0 ? " {$skipped} skipped." : ''))
                                ->success()
                                ->send();
                        })
                        ->deselectRecordsAfterCompletion(),
                    ...
                    BulkAction::make('bulk_return')
                        ...
                        ->form(fn (): array => [
                            Toggle::make('restore_stock')
                                ...
                                ->default(true)
                                ->live()
                                ->helperText('Turn on to restore full released quantities to inventory for all selected records.')
                                ->columnSpanFull(),
                        ])
                        ->action(function (Collection $records, array $data) {
                            ...
                            Notification::make()
                                ->title("{$returned} issuance(s) returned. {$stockMsg}")
                                ->success()
                                ->send();
                        })
                        ->deselectRecordsAfterCompletion(),
